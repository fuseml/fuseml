...
As a stateful application, fuseml-core needs to save its state in a form of persistent storage so it can survive service restarts without losing its current state.
...
In its current version, fuseml-core does not provide a persistent storage any of it components, except for `Codesets` and `Projects` which is able to save/load its state from a Git solution (Gitea). The rest of the components (`Application`, `Workflow` and `Runnable`) relies on a basic implementation of a in-memory storage, meaning that in case of a service restart fuseml-core would lose any information related to those components.
...
![fuseml-core](fuseml-core-storage.png?raw=true)
